...
function fixIndexHtmlPaths() {
  ...
  return {
    ...
    transformIndexHtml(html) {
      ...
      // Если есть /src/main.js, это означает, что Vite еще не обработал его
      // В этом случае просто возвращаем HTML как есть - Vite обработает его позже
      return html
    },
    writeBundle(options, bundle) {
      ...
      try {
        ...
        // ВАЖНО: НЕ исправляем пути к /src/ здесь, так как это может сломать работу
        // Если Vite не обработал /src/main.js, мы уже исправили это выше
        // Остальные пути к /src/ не должны быть в собранном HTML
